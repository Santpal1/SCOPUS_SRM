...
import dev1 from "../assets/react.svg";
import dev2 from "../assets/react.svg";
import dev3 from "../assets/react.svg";
...
const faqData = [
  {
    q: "How do I add a new faculty member?",
    a: "Go to the Dashboard, click on 'Faculty List', and use the 'Add Faculty' button.",
  },
  {
    q: "Can I export faculty performance reports?",
    a: "Yes, you can export reports in PDF or Excel from the Dashboard's 'Reports' section.",
  },
  {
    q: "Who can access research paper statistics?",
    a: "Only users with admin or research privileges can access detailed research paper stats.",
  },
];
...
const FacultyLandingPage = () => {
  // Animated stats
  const departments = useCountUp(200);
  const faculties = useCountUp(20000);
  const users = useCountUp(2000);
  const papers = useCountUp(200);
  ...
  return (
    <div className={styles.fullPageContainer}>
      ...
      >
        <div className={styles.logo}>SRM SP</div>
        <div className={styles.navLinks}>
          <motion.a whileHover={{ scale: 1.1, color: "#ffe066" }} href="#">
            Dashboard
          </motion.a>
          <motion.a whileHover={{ scale: 1.1, color: "#ffe066" }} href="#">
            Faculty List
        ...
        <div className={styles.authButtons}>
          <motion.button className={styles.login} whileHover={{ scale: 1.05 }}>
            Login
          </motion.button>
          <motion.button className={styles.signup} whileHover={{ scale: 1.05 }}>
            SignUp
          </motion.button>
        </div>
      </motion.div>
      ...
      <div className={styles.mainContent}>
        ...
        {/* FAQ SECTION - no extra spacing above */}
        <motion.section
            ...
            {faqData.map((item, idx) => (
              <div className={styles.faqItem} key={idx}>
                ...
                <motion.div
                  ...
                  initial={false}
                  animate={{
                    maxHeight: faqOpen[idx] ? 200 : 0,
                    opacity: faqOpen[idx] ? 1 : 0,
                  }}
                  transition={{ duration: 0.3 }}
                  style={{ overflow: "hidden" }}
                >
                  {faqOpen[idx] && <div>{item.a}</div>}
                </motion.div>
              </div>
            ))}
          </div>
        ...
        >
          <div className={styles.footerContent}>
            ...
            <div>
              <p>
                Welcome to our platform! Here you can discover detailed
                analytics, collaborate with peers, and access resources to help
                you excel. Our commitment is to empower educators and foster a
                thriving academic community. Stay tuned for more updates and
                features coming soon.
              </p>
              <p>
                For further inquiries, please reach out to our support team or
                check back as we expand our documentation and resources to serve
                you better.
              </p>
            </div>
          </div>
