...
scopus_ids_from_db = [row[0] for row in cursor.fetchall()]  
# Function to insert user (author) details
def insert_user(scopus_id, name, docs_count):
    cursor.execute("""
...
# Function to insert paper details (including author and affiliation info)
...
all_ids_set = set(scopus_ids)
filled_ids_set = set(scopus_ids_from_db)

# Find missing Scopus IDs
missing_ids = all_ids_set - filled_ids_set
print(missing_ids)

for scopus_id in missing_ids:
    ...
    if my_auth.read(client):
        ...
        print(f"Total documents for {scopus_id}: {docs_count}")
        
        insert_user(scopus_id, my_auth.full_name, docs_count)
    else:
        print(f"Failed to read author data for {scopus_id}.")
        continue
    
    # Fetch documents for the author
